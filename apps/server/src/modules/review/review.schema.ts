...
export const createReviewSchema = z
  .object({
    ...
    pros: z
      .array(
        z.string().trim().min(1, 'Pro cannot be empty').max(200, 'Each pro must be 200 characters or fewer')
      )
    ...
    cons: z
      .array(
        z.string().trim().min(1, 'Con cannot be empty').max(200, 'Each con must be 200 characters or fewer')
      )
  ...
  .refine(
    (d) => new Set(d.pros).size === d.pros.length,
    { message: 'Pros list contains duplicate entries', path: ['pros'] }
  )
  .refine(
    (d) => new Set(d.cons).size === d.cons.length,
    { message: 'Cons list contains duplicate entries', path: ['cons'] }
  )
  .refine(
    (d) => d.title.length >= 5,
    { message: 'Title is too short after sanitization (min 5 characters)', path: ['title'] }
  )
  .refine(
    (d) => d.body.length >= 20,
    { message: 'Review body is too short after sanitization (min 20 characters)', path: ['body'] }
  )
